...
class FirebaseApiRepository {
  ...
  Future<void> SetJwtToken(String token) async{
    ...
    SharedPrefs().username = "asas";
    print("Your JWT is :"+SharedPrefs().username);
  }

  Future<String> GetCurrentJWTToken() async{
    SharedPreferences prefs = await SharedPreferences.getInstance();
    String jwt_token= SharedPrefs().username;
  ...
  Future<bool> GetToken(String mobileNumber,String userEnteredOTP) async{
    var headers = {
      'Content-Type': 'application/json'
    };
  ...
  /*Future<bool> IPV_Video_Upload(List<int> byteFormatOfFile,File videoFile) async{
    String jwt_token= await GetCurrentJWTToken();
  ...
  Future<Map> Digio_eSign_Document_Upload() async{
    Map valueMap = {};
    String jwt_token= await GetCurrentJWTToken();
    ...
    if (response.statusCode == 200) {
      ...
      String esignToken = valueMap["res_Output"][0]["result_Extra_Key"];
      print("Your Esign Token is : "+esignToken);
  ...
  Future<void> Send_IPV_URL() async{
    String jwt_token= await GetCurrentJWTToken();
    print("Calling Send_IPV_URL LOCAL Using API"+jwt_token);

    String lead_id = await GetLeadId();
    print("Send_IPV_URL LOCAL for Lead ID : "+lead_id);

    var headers = {
      'Authorization': 'Bearer $jwt_token',
      'Content-Type': 'application/json'
    };
    var request = http.Request('POST', Uri.parse('$BASE_API_LINK_URL/api/in_person_verification/Send_IPV_URL'));
    request.body = json.encode({
      "lead_Id": "$lead_id",
    });
    request.headers.addAll(headers);
  ...
  Future<void> Video_Upload_DIO(File? file) async {
    String jwt_token= await GetCurrentJWTToken();
    ...
    Response response = await dio.post('$BASE_API_LINK_URL/api/in_person_verification/Video_Upload', data: formData);
    print(response.data);
  }
  ...
  Future<void> RazorPaymentStatus(String amountPayed,String merchantID ,String paymentID,String signature,String depositoryCharge) async{
    String jwt_token= await GetCurrentJWTToken();
